...
import { getProductPath, sortStorefrontProducts, toLandingProduct } from "../../lib/product";
import {
  defaultTransition,
  fadeUp,
  scaleIn,
...
function ProductCardSkeleton() {
  return (
    <div className="product-ornate-frame h-full animate-pulse">
      <div className="px-2.5 py-3 sm:px-3 sm:py-3.5 lg:px-3.5 lg:py-4">
        <div className="flex flex-col items-center gap-4 sm:flex-row sm:items-start sm:gap-2.5 lg:gap-3">
          <div className="h-44 w-full max-w-[10.5rem] rounded-lg bg-olive-800/40 sm:h-52 sm:w-[44%] sm:max-w-none lg:w-[46%]" />
          <div className="flex w-full flex-1 flex-col items-center gap-3 sm:items-start">
            <div className="h-6 w-3/4 rounded bg-olive-800/40" />
            <div className="h-4 w-full rounded bg-olive-800/30" />
            <div className="h-4 w-5/6 rounded bg-olive-800/30" />
            <div className="h-9 w-28 rounded-full bg-olive-800/40" />
          </div>
        </div>
      </div>
    </div>
...
export default function Products() {
  const { items, status, error } = useStorefrontProducts();
  const products = sortStorefrontProducts(items).map(toLandingProduct);
  ...
  return (
    <section id="products" className="relative overflow-hidden bg-olive-900">
      ...
      <div className="container-ashwini relative z-10 section-padding">
        ...
        {status === "loading" ? (
          <div className="mx-auto grid w-full max-w-6xl grid-cols-1 gap-6 min-[480px]:grid-cols-2 lg:max-w-7xl lg:grid-cols-3 lg:gap-6 xl:gap-7">
        ...
        {status === "error" ? (
          <p className="text-center text-body-sm text-cream-300">{error}</p>
        ) : null}

        {status === "success" && products.length === 0 ? (
          <p className="text-center text-body-sm text-cream-300">
